<template>
  <div class="container">
    <div class="top-head"><Icon class="top-left-icon" size="20" type="ios-home-outline" /><span>当前位置:</span><span>系统设置</span>&gt;<span>系统记录</span></div>
    <div class="search">
      <Input icon="ios-search-outline"   @on-change="search" class="search-input" placeholder="请输入批次名字" style="width: auto;visibility: hidden" />
      <Button type="primary" class="add" @click="showAddMessage">发送系统消息</Button>
    </div>
    ...
    <Modal class="change-modal"
           ...
           @on-cancel="cancelAdd"
           :mask-closable="false">
      <p><span class="first-title">标题:</span><Input v-model="addData.title" size="large" placeholder="标题" style="width: 200px" /></p>
    ...
    export default {
      ...
      methods: {
        ...
        getTypeList(){
          ...
          var userId=sessionStorage.getItem('userId');
          $.ajax({
            url: 'http://api-erp.juwenjiaoyu.com:9000/api/user/'+userId+'/system/selectSystemMessages',
            data: {
        ...
        addMessage(index){
          ...
          var userId=sessionStorage.getItem('userId');
          $.ajax({
            url: 'http://api-erp.juwenjiaoyu.com:9000/api/user/'+userId+'/system/addSystemMessages',
            data: {
