<template>
  <div class="container">
    <div class="top-head"><Icon class="top-left-icon" size="20" type="ios-home-outline" /><span>当前位置:</span><span>系统管理</span>&gt;<span>注册点管理</span></div>
    <div class="search">
      ...
      <Button type="primary" class="add" @click="showAddMessage">添加注册点</Button>
    </div>
    ...
    <Modal class="change-modal"
           ...
           @on-cancel="cancel"
           :mask-closable="false">
      <p><span class="first-title">注册点名称:</span><Input v-model="changeMessageData.name" size="large" placeholder="注册点名称" style="width: 200px" /></p>
    ...
    <Modal class="change-modal"
           ...
           @on-cancel="cancelAdd"
           :mask-closable="false">
      <p><span class="first-title">注册点:</span><Input v-model="addData.name" size="large" placeholder="注册点名称" style="width: 200px" /></p>
      <p><span class="first-title">备注:</span><Input v-model="addData.descriptions" size="large" placeholder="批次说明" style="width: 200px" /></p>
    </Modal>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      remove (index) {
        this.$Modal.confirm({
          ...
          onOk: () => {
            ...
            var _this=this;
            $.ajax({
              url: 'http://api-erp.juwenjiaoyu.com:9000/api/user/'+userId+'/system/delRegisterPoint',
              data: {
      ...
      addMessage(index){
        ...
        var userId=sessionStorage.getItem('userId');
        $.ajax({
          url: 'http://api-erp.juwenjiaoyu.com:9000/api/user/'+userId+'/system/addRegisterPoint',
          data: {
            name :_this.addData.name,
            descriptions:_this.addData.descriptions,
          },
          beforeSend: function(request) {
            request.setRequestHeader("x-access-token", token);
          },
