<template>
  <div class="container">
    <div class="top-head"><Icon class="top-left-icon" size="20" type="ios-home-outline" /><span>当前位置:</span><span>教务管理</span>&gt;<span>学生列表</span></div>
    <div class="search">
      <Input suffix="ios-search" class="search-input" v-model="searchType"  @on-change="search" placeholder="请输入老师名字" style="width: auto" />
    ...
    <Drawer title="学习记录" :closable="false" v-model="studyMd">
      <Timeline>
        <TimelineItem v-for="(item,index) in studyData">
          <p class="time">{{item.studyTime}}</p>
          <p class="content">{{item.studyContent}}</p>
        </TimelineItem>
      </Timeline>
    </Drawer>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      getStudentList(){
        ...
        var userId=sessionStorage.getItem('userId');
        $.ajax({
          url: 'http://api-erp.juwenjiaoyu.com:9000/api/user/'+userId+'/teacher/selectStudentInfos',
          data: {
